...
I hope this will be a helpful grounding to now go through the precise ways that an LLM, or even an agentic coding agent is not like a junior programmer.
...
There are many techniques for giving these agents memory but the 2 easiest and most commonly used are:
...
Remember though, that the "knowledge" captured in the parameters for a model are based on its training data. So if your task requires specialised knowledge, you are going to have to feed that knowledge to the model EVERY SINGLE TIME it is needed (see point 1).
...
This has an important implication. If you are not learning how these tools work, and when they work well, you will be outperformed by those that do. In the future it will be expected that certain classes of tasks are not done by hand.

## 4. World model

Coming back to the output generation. We already covered that the "knowledge" is encoded in a "lossy" way using the arrays of numbers in the parameters. These weighting numbers combined with the runtime configuration of things like *temperature* determine the next token generated until the full output is returned.

It is important to notice that in this stochastic model is nothing that represents a model of the real world, other than that the training data was created in the real world. The LLM has no way of testing its output with reality, because it *understands neither its output nor the real world*.
...
So our junior programmer is different to these models in a useful way. They can reality test and reason in a way that the agentic agent cannot.
...
And remember. They are token generators, not social primates.
