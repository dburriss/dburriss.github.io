...
The site SHALL render legacy category and tag pages as static HTML pages that perform a meta `http-equiv="refresh"` redirect to the most relevant topic page. When no mapping exists, the redirect SHALL point to the topics overview page.

#### Scenario: Legacy category redirects to mapped topic
- **WHEN** a reader visits `/categories/{category}/`
- **THEN** a meta refresh redirects to `/topics/{mapped-id}/`

#### Scenario: Legacy tag redirects to mapped topic
- **WHEN** a reader visits `/tags/{tag}/`
- **THEN** a meta refresh redirects to `/topics/{mapped-id}/`
